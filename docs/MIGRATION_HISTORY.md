...
| `20260826234754_add_clinical_professional_roles.sql` | `20260827001020` — `add_clinical_professional_roles` |
| `20260827161226_add_microareas_and_population_counts.sql` | `20260827180933` — `add_microareas_and_population_counts` |
...
O arquivo foi criado pela Supabase CLI e aplicado no projeto Supabase como `20260827001020 — add_clinical_professional_roles`. A verificação posterior confirmou os papéis clínicos no histórico remoto.
...
O arquivo não cadastra pessoas, famílias identificáveis ou dados clínicos. Perfis ACS existentes são vinculados somente quando equipe e código textual permitem correspondência segura; casos ambíguos permanecem sem identificador para conferência administrativa.

A migration foi aplicada no projeto Supabase como `20260827180933 — add_microareas_and_population_counts`. A verificação posterior confirmou tabela criada, RLS habilitada, quatro policies, acesso anônimo bloqueado e o ACS existente vinculado à sua microárea sem inventar total populacional.

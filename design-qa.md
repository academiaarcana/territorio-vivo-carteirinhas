# Design QA — Território e Carteirinhas em Campo
...
- Implementação: `src/pages/territory.js`, `src/styles/field-territory.css`, `src/pages/cards.js` e `src/styles/field-cards.css`.
- Captura da implementação: indisponível.
- Viewport-alvo: desktop 1440 × 1024 CSS px, densidade 1×; reflow previsto em 760 px.
- Estados-alvo: rotas `#/app/territorio` e `#/app/carteirinhas` com um perfil autenticado e vínculo territorial válido.
- Interações prioritárias: filtrar achados e rede, cadastrar e gerenciar achados conforme permissão; buscar e filtrar modelos, abrir o editor, preencher lote, ajustar acessibilidade e iniciar PDF/impressão.
...
- Espaçamento e ritmo: Território usa faixa introdutória, área de trabalho assimétrica e métricas leves; Carteirinhas usa biblioteca em três colunas e editor com formulário e prévia. As proporções finais dependem de captura.
...
- Texto e conteúdo: a redação permanece não clínica, não pessoal e orientada à equipe; Carteirinhas mantém a declaração explícita de dados temporários e não persistentes. Dados e permissões não foram alterados.
...
Obter capturas reais das rotas autenticadas Território e Carteirinhas em desktop e celular, verificar as interações e o console e comparar as implementações com a referência em uma composição conjunta.

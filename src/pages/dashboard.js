...
export function renderDashboard({ state }) {
  ...
  const missingTerritory = !master && (!profile.municipality_code || !profile.unit_cnes);
  const hero = master
    ? '<section class="hero-panel"><p class="eyebrow">Administração superior</p><h2>Visão integrada da rede cadastrada.</h2><p>Administre municípios, unidades, equipes, acessos e território sem assumir o vínculo operacional de um ACS.</p><div class="actions"><button class="button primary" data-go="/app/gestao">Abrir gestão da rede</button><button class="button" data-go="/app/aprovacoes">Revisar aprovações</button><button class="button" data-go="/app/territorio">Abrir território</button></div></section>'
  ...
  const accountSummary = master
    ? `<article class="panel"><h3>Conta e escopo</h3><dl class="summary-list"><div><dt>Responsável</dt><dd>${escapeHtml(profile.full_name || '—')}</dd></div><div><dt>Conta</dt><dd>${escapeHtml(roleLabel(profile))}</dd></div><div><dt>Escopo</dt><dd>${escapeHtml(territory)}</dd></div><div><dt>Status</dt><dd>Administração superior ativa</dd></div></dl><button class="link-button" data-go="/app/perfil">Configurar conta</button></article>`
  ...
  const content = `
    ...
    <section class="dashboard-grid">
      ...
      ${management ? `<article class="panel"><h3>${master ? 'Gestão da rede' : 'Gestão da UBS'}</h3><p>${master ? 'Administre municípios, unidades, equipes, papéis e perfis.' : 'Administre dados, equipes e profissionais somente da sua UBS.'}</p><button class="link-button" data-go="/app/gestao">Abrir gestão</button></article>` : ''}
    </section>`;
  return appLayout({ title: master ? 'Painel Master' : 'Início', subtitle: master ? 'Administração geral da rede cadastrada.' : 'Visão rápida do trabalho territorial.', activePath: '/app/inicio', content });
}

...
import { isMaster, isMasterAccount, roleLabel } from '../core/permissions.js';
...
export function renderProfilePage({ state }) {
  ...
  const superiorTitle = masterAccount ? 'Master / Desenvolvimento e preferências de materiais' : 'Gestor Municipal e preferências de materiais';
  const superiorIntro = masterAccount
    ? 'Esta conta técnica administra toda a rede e protege a configuração superior de acessos. Os dados territoriais abaixo servem apenas como contexto padrão para materiais.'
    : 'Sua conta de Gestor Municipal administra toda a rede cadastrada. Os dados territoriais abaixo servem apenas como contexto padrão para materiais e não limitam seu acesso.';
  const content = `
    <section class="panel"><p class="eyebrow">${masterAccount ? 'Master / Desenvolvimento' : networkAdmin ? 'Gestão municipal' : 'Perfil reutilizável'}</p><h2>${networkAdmin ? superiorTitle : 'Meu perfil e meu território'}</h2><p>${networkAdmin ? superiorIntro : 'Estes dados são salvos no Supabase e reaproveitados nos módulos. Informações de famílias não entram aqui.'}</p>
      <div class="role-notice"><strong>${networkAdmin ? escapeHtml(roleLabel(p)) : escapeHtml(roleLabel(p))}</strong>${scopeLocked ? '<span>Seu município, UBS, equipe e microárea foram validados pela gestão. Para corrigir o vínculo, solicite a atualização à administração.</span>' : `<span>${masterAccount ? 'Administração técnica' : 'Gestão municipal'} com acesso a municípios, UBS, equipes, profissionais e território. O contexto padrão de materiais não altera esse escopo.</span>`}</div>
      <form id="profile-form" class="profile-sections">
        <fieldset><legend>${networkAdmin ? 'Responsável pela conta' : 'Profissional'}</legend><div class="form-grid"><label>Nome completo<input name="full_name" value="${escapeHtml(p.full_name || '')}" required maxlength="160"></label>${networkAdmin ? '' : `<label>Microárea<input name="microarea" value="${escapeHtml(p.microarea || '')}" maxlength="40" placeholder="Ex.: 08" disabled aria-disabled="true"></label>`}<label>Telefone / contato institucional<input name="acs_phone" value="${escapeHtml(p.acs_phone || '')}" maxlength="80"></label></div>${scopeLocked ? '<p class="field-hint">Microárea faz parte do vínculo territorial aprovado e só pode ser alterada pela gestão.</p>' : ''}</fieldset>
        <fieldset><legend>${networkAdmin ? 'Contexto padrão para materiais' : 'Território'}</legend>${networkAdmin ? `<p class="field-hint">Opcional. Ajuda a preencher carteirinhas e materiais, mas ${masterAccount ? 'a conta Master / Desenvolvimento' : 'o Gestor Municipal'} continua com acesso a toda a rede cadastrada.</p>` : ''}<div class="form-grid"><label>Município<select name="municipality_code" id="profile-municipality" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></select></label><label>Unidade<select name="unit_cnes" id="profile-unit" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></select></label><label>Equipe<select name="team_id" id="profile-team" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></select></label><label id="profile-custom-team-wrap" hidden>Nome da equipe<input name="team_name_custom" maxlength="120" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></label></div><div id="profile-source" class="source-note"></div></fieldset>
...
export async function mountProfilePage({ root, state }) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const microarea = isMaster(state.profile) ? '' : (state.profile?.microarea || '');

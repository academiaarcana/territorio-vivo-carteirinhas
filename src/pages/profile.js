...
export function renderProfilePage({ state }) {
  ...
  const scopeLocked = !master;
  const content = `
    <section class="panel"><p class="eyebrow">${master ? 'Administração superior' : 'Perfil reutilizável'}</p><h2>${master ? 'Conta Master e preferências de materiais' : 'Meu perfil e meu território'}</h2><p>${master ? 'Sua conta administra toda a rede cadastrada. Os dados territoriais abaixo servem apenas como contexto padrão para materiais e não limitam seu acesso.' : 'Estes dados são salvos no Supabase e reaproveitados nos módulos. Informações de famílias não entram aqui.'}</p>
      <div class="role-notice"><strong>${master ? 'Conta Master — Administração geral' : escapeHtml(roleLabel(p))}</strong>${scopeLocked ? '<span>Seu município, UBS, equipe e microárea foram validados pela gestão. Para corrigir o vínculo, solicite a atualização à administração.</span>' : '<span>Acesso superior a municípios, UBS, equipes, profissionais e território. O contexto padrão de materiais não altera esse escopo.</span>'}</div>
      <form id="profile-form" class="profile-sections">
        <fieldset><legend>${master ? 'Responsável pela conta' : 'Profissional'}</legend><div class="form-grid"><label>Nome completo<input name="full_name" value="${escapeHtml(p.full_name || '')}" required maxlength="160"></label>${master ? '' : `<label>Microárea<input name="microarea" value="${escapeHtml(p.microarea || '')}" maxlength="40" placeholder="Ex.: 08" disabled aria-disabled="true"></label>`}<label>Telefone / contato institucional<input name="acs_phone" value="${escapeHtml(p.acs_phone || '')}" maxlength="80"></label></div>${scopeLocked ? '<p class="field-hint">Microárea faz parte do vínculo territorial aprovado e só pode ser alterada pela gestão.</p>' : ''}</fieldset>
        <fieldset><legend>${master ? 'Contexto padrão para materiais' : 'Território'}</legend>${master ? '<p class="field-hint">Opcional. Ajuda a preencher carteirinhas e materiais, mas a Conta Master continua com acesso a toda a rede cadastrada.</p>' : ''}<div class="form-grid"><label>Município<select name="municipality_code" id="profile-municipality" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></select></label><label>Unidade<select name="unit_cnes" id="profile-unit" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></select></label><label>Equipe<select name="team_id" id="profile-team" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></select></label><label id="profile-custom-team-wrap" hidden>Nome da equipe<input name="team_name_custom" maxlength="120" ${scopeLocked ? 'disabled aria-disabled="true"' : ''}></label></div><div id="profile-source" class="source-note"></div></fieldset>
  ...
  return appLayout({ title: master ? 'Configurações da Conta Master' : 'Meu perfil', subtitle: master ? 'Administração superior e preferências de materiais.' : 'Dados profissionais e vínculo territorial.', activePath: '/app/perfil', content });
}
